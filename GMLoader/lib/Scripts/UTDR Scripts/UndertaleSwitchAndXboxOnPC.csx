...
importGroup.QueueFindReplace("gml_Object_obj_time_Create_0", "if (global.osflavor >= 4)", "if (true)");
...
importGroup.QueueFindReplace("gml_Object_obj_time_Step_1", "global.osflavor <= 2", "false");
importGroup.QueueFindReplace("gml_Object_obj_time_Step_1", $"os_type == {os_switch}", "true");
...
""");
importGroup.QueueFindReplace("gml_Object_obj_joypadmenu_Draw_0", """
    global.button0 = 2;
    global.button1 = 1;
    global.button2 = 4;
""", """
...
importGroup.QueueFindReplace("gml_Script_control_update", "else if (obj_time.j_ch > 0)", "else if (false)");
importGroup.QueueFindReplace("gml_Script_control_update", "global.osflavor >= 4", "obj_time.j_ch > 0");

// Make the Joystick Menu use the new gamepad functions.
importGroup.QueueFindReplace("gml_Object_obj_joypadmenu_Create_0", "joystick_has_pov(obj_time.j_ch)", "true");
importGroup.QueueFindReplace("gml_Object_obj_joypadmenu_Draw_0", "joystick_has_pov(obj_time.j_ch)", "true");
// TODO: gamepad_button_count(obj_time.j_ch - 1) might work better?
importGroup.QueueFindReplace("gml_Object_obj_joypadmenu_Draw_0", "joystick_buttons(obj_time.j_ch)", "11 + 1");
importGroup.QueueFindReplace("gml_Object_obj_joypadmenu_Draw_0", "joystick_check_button(obj_time.j_ch, i)", "gamepad_button_check(obj_time.j_ch - 1, gp_face1 + i)");
for (var i = 0; i < 3; i++)
{
    importGroup.QueueFindReplace("gml_Object_obj_joypadmenu_Draw_0", $"global.button{i} = i", $"global.button{i} = gp_face1 + i");
    importGroup.QueueFindReplace("gml_Object_obj_joypadmenu_Draw_0", $"string_hash_to_newline(global.button{i})", $"string(global.button{i} - gp_face1)");
}
...
foreach (char half in new[] {'l', 'r'})
{
    ...
    {
        importGroup.QueueFindReplace($"gml_Script_scr_heart{half}_hold{side}", "global.osflavor <= 2 && ", "");
        importGroup.QueueFindReplace($"gml_Script_scr_heart{half}_hold{side}", "global.osflavor >= 4 && ", "");
    }
}
...
if (ScriptQuestion("Enable the Dog Shrine?"))
{
    if (isXbox)
    {
        ...
        importGroup.QueueFindReplace("gml_Object_obj_kitchenchecker_Create_0", "global.osflavor == 4 || global.osflavor == 5 || global.osflavor == 6", "true");
        importGroup.QueueFindReplace("gml_Object_obj_kitchenchecker_Alarm_2", "(global.osflavor == 4 || global.osflavor == 5 || global.osflavor == 6) && ", "");
        // This patch is unnecessary in the NX version, as the door will get you into the ruined Dog Shrine.
        importGroup.QueueFindReplace("gml_Object_obj_doorXmusicfade_Alarm_2", "if (global.osflavor == 6)", "else");

        // Enable the donation box trash in Waterfall.
        importGroup.QueueFindReplace("gml_Object_obj_npc_room_Create_0", "global.osflavor != 4 && global.osflavor != 6", "false");
    }
    else
    {
        // This enables the entrance to the Dog Shrine.
        importGroup.QueueFindReplace("gml_Object_obj_kitchenchecker_Create_0", "global.osflavor == 4 || global.osflavor == 5", "true");
        importGroup.QueueFindReplace("gml_Object_obj_kitchenchecker_Alarm_2", "(global.osflavor == 4 || global.osflavor == 5) && ", "");

        // Enable the donation box trash in Waterfall.
        importGroup.QueueFindReplace("gml_Object_obj_npc_room_Create_0", "(global.osflavor != 4 && global.osflavor != 5) || ", "");
    }
}
